...
Future<Widget> insertRecord(String barcode, BuildContext context) {
  return showDialog(
    context: context,
    builder: (BuildContext context) {
      final GlobalKey<FormState> _formKey = GlobalKey<FormState>();
      final List<TextEditingController> _record = new List(11);
                ...
                RaisedButton(
                  ...
                  onPressed: () async {
                    if(_formKey.currentState.validate()) {
                      ...
                      String line = '$barcode;${_record[0].text};${_record[1].text};${_record[2].text};${_record[3].text};${_record[4].text};${_record[5].text};${_record[6].text};${_record[7].text};${_record[8].text};${_record[9].text};${_record[10].text}';
                      bool isSended = await sendTo(line, '192.168.1.42', 3000);
                      if(isSended) {
                        log('Cose inserite:',type: 'Form Check');
                        log('Barcode: $barcode',type: 'Form Check');
                        log('Descrizione: ${_record[0].text}',type: 'Form Check');
                        log('Quantità: ${_record[1].text}',type: 'Form Check');
                        log('Reparto: ${_record[2].text}',type: 'Form Check');
                        log('Scorta: ${_record[3].text}',type: 'Form Check');
                        log('Fornitore: ${_record[4].text}',type: 'Form Check');
                        log('Costo: ${_record[5].text}',type: 'Form Check');
                        log('Prezzo 1: ${_record[6].text}',type: 'Form Check');
                        log('Prezzo 2: ${_record[7].text}',type: 'Form Check');
                        log('Prezzo 3: ${_record[8].text}',type: 'Form Check');
                        log('Note: ${_record[9].text}',type: 'Form Check');
                        log('Immagini: ${_record[10].text}',type: 'Form Check');
